//! Canonical text-processing utilities for HydeClaw.
//!
//! # Responsibility
//!
//! Currently: paragraph/sentence-aware text splitting (`split_text`) used by
//! the memory indexing pipeline.
//!
//! # NOT here: TTS normalization
//!
//! Text normalization for TTS (expanding numbers to words, transliterating
//! English to Cyrillic, stripping markdown) lives in `toolgate/normalize.py`.
//! That logic is *intentionally separate* from indexing:
//!
//! - TTS wants "Python" read as "Пайтон" so the synthesizer pronounces it.
//! - Indexing wants "Python" embedded as "Python" so a user query for
//!   "Python" matches.
//!
//! Do NOT add TTS-style normalization here. If memory indexing grows its own
//! cleanup (markdown strip, whitespace normalization), add it as a separate
//! module in this crate with embedding-friendly semantics.
