...
async fn migration_026_idempotent_on_second_run() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn migration_026_preserves_timeout_zero() {
    let harness = TestHarness::new().await.unwrap();
    let pool = harness.pool();
    sqlx::query(
        "INSERT INTO providers (id, name, type, provider_type, enabled, options)
...
/// `timeout_secs`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn migration_026_preserves_hand_edited_timeouts_with_legacy_key() {
    let harness = TestHarness::new().await.unwrap();
    let pool = harness.pool();
