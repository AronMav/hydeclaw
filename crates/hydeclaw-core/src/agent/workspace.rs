...
pub fn build_system_prompt(
    ...
    prompt.push_str(concat!(
        ...
        "2. Do other useful work (fetch data, run tools) while agents process\n",
        "3. Check status INFREQUENTLY: `agent(action=\"status\")` — max 2-3 times total, not in a tight loop\n",
        "4. Agents need 1-3 minutes to complete. Do NOT poll rapidly — each status check costs an iteration\n",
        "5. When status is idle with last_result → read and synthesize the result\n",
        "6. If agent is still processing after 3 minutes → kill and do the task yourself\n\n",
        "IMPORTANT: Do NOT waste iterations on rapid polling. Check status 2-3 times max, with other work between checks.\n",
        "Multiple agents: check ALL with one status call (omit target), not individually.\n",
