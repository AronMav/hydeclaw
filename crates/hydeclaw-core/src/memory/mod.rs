...
impl MemoryStore {
    pub fn new(db: sqlx::PgPool, config: &MemoryConfig, toolgate_url: &str) -> Self {
        let store = Arc::new(PgVectorStore::new(db));
        Self::with_store(store, config, toolgate_url)
    }

    /// Create a MemoryStore with a custom VectorStore implementation.
    pub fn with_store(
        store: Arc<dyn VectorStore>,
        config: &MemoryConfig,
        toolgate_url: &str,
    ...
    async fn do_initialize(&self) -> Result<()> {
        ...
        let existing_dim = self.store.get_existing_embedding_dim().await;
        ...
        if let Some(old_dim) = existing_dim
            && old_dim as u32 != dim {
                ...
                self.store.drop_hnsw_index().await?;
            }

        // 3. Ensure HNSW index with correct dimension
        self.store.ensure_hnsw_index(dim).await?;
    ...
    pub async fn embed(&self, text: &str) -> Result<Vec<f32>> {
        ...
        if self.embed_dim.load(Ordering::Relaxed) == 0 {
            ...
            if let Err(e) = self.store.ensure_hnsw_index(dim).await {
        ...
        if !short_items.is_empty() {
            ...
            for (i, &(idx, content, source, pinned)) in short_items.iter().enumerate() {
                ...
                self.store.insert_chunk(
                    &id, content, &vec_str, source, pinned, &lang, None, 0,
                ).await
    ...
    async fn search_semantic(&self, query: &str, limit: usize) -> Result<Vec<MemoryResult>> {
        ...
        let mut candidates = self.store.search_semantic(
        ...
        self.store.touch_accessed(&ids).await;
    ...
    pub async fn search_fts(&self, query: &str, limit: usize) -> Result<Vec<MemoryResult>> {
        ...
        let results = self.store.search_fts(
            query, limit as i64, &lang,
        )
        .await?;
    ...
    pub async fn recent(&self, limit: i64) -> Result<Vec<MemoryResult>> {
        self.store.fetch_recent(limit).await
    }
    ...
    pub async fn index(&self, content: &str, source: &str, pinned: bool) -> Result<String> {
        ...
        if chunks.len() == 1 {
            ...
            let id = uuid::Uuid::new_v4().to_string();
            self.store.insert_chunk(
                &id, &chunks[0], &vec_str, source, pinned, &lang, None, 0,
            ).await?;
            return Ok(id);
        }
        ...
        for (i, (chunk, embedding)) in chunks.iter().zip(embeddings.iter()).enumerate() {
            ...
            self.store.insert_chunk(
                &id, chunk, &vec_str, source, pinned, &lang, parent, i as i32,
            ).await?;
        }
    ...
    pub async fn get(
        ...
        match (chunk_id, source) {
            ...
            (None, Some(src)) => {
                self.store.get_chunks_by_source(src, limit as i64).await
            }
            (None, None) => {
                self.store.get_chunks_recent(limit as i64).await
            }
        }
    }
    ...
    pub async fn rebuild_fts(&self) -> Result<u64> {
        let lang = self.validated_fts_language()?;
        let rows = self.store.rebuild_fts(&lang).await?;
