...
/// A single chunk eligible for compression.
...
pub async fn fetch_compressible_groups(
    db: &PgPool,
    age_days: u32,
) -> anyhow::Result<Vec<CompressibleGroup>> {
    // Build the interval string safely — age_days is u32, not user input.
    let sql = format!(
        "SELECT id, agent_id, content, category, topic \
         FROM memory_chunks \
         WHERE pinned = false AND archived = false \
           AND created_at < now() - interval '{age_days} days' \
           AND topic IS NOT NULL \
         ORDER BY agent_id, topic, created_at ASC"
    );

    let rows = sqlx::query(&sql)
        .fetch_all(db)
        .await
        .context("failed to fetch compressible chunks")?;

    use sqlx::Row;
    let mut groups: Vec<CompressibleGroup> = Vec::new();
    ...
    for row in &rows {
        ...
        // Find existing group or push new one
        if let Some(g) = groups
            .iter_mut()
            .rev()
            .find(|g| g.agent_id == agent_id && g.topic == topic)
        {
            g.chunks.push(CompressibleChunk { id, content, category });
        } else {
            groups.push(CompressibleGroup {
                agent_id,
                topic,
                chunks: vec![CompressibleChunk { id, content, category }],
            });
        }
    }
...
/// Archive chunks by setting archived = true. Returns count of rows affected.
pub async fn archive_chunks(db: &PgPool, chunk_ids: &[uuid::Uuid]) -> anyhow::Result<u64> {
    if chunk_ids.is_empty() {
        return Ok(0);
    }
    let res = sqlx::query("UPDATE memory_chunks SET archived = true WHERE id = ANY($1)")
        .bind(chunk_ids)
        .execute(db)
        .await
        .context("failed to archive memory chunks")?;
    Ok(res.rows_affected())
}
