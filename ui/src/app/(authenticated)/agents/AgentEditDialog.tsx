      ...
      <DialogContent className="max-h-[90vh] overflow-hidden p-0 border-border shadow-2xl max-w-[95vw] sm:max-w-xl rounded-xl">
        ...
        <ScrollArea className="max-h-[60vh] sm:max-h-[70vh]">
          <div className="px-5 py-3 space-y-3">
            {/* Core */}
            <Section title={t("agents.section_core")}>
              {/* Row 1: Icon + Name + Language */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      if (v === "__none__") {
                        ...
                        upd({
                          providerConnection: v,
                          provider: sp?.provider_type ?? "",
                ...
                <Field label={t("agents.field_top_k_tools")}>
                  <Input
                    ...
                    min="1"
                    max="50"
                    value={form.maxToolsInContext}
                    placeholder={t("agents.placeholder_all")}
                    className="bg-background border-border font-mono text-sm h-8"
                    onChange={(e) => upd({ maxToolsInContext: e.target.value })}
                  />
                </Field>
            ...
            {/* Tool Policy */}
            <SwitchSection
              title={t("agents.section_tool_policy")}
              enabled={form.toolsEnabled}
              onToggle={(v) => upd({ toolsEnabled: v })}
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium text-muted-foreground">{t("agents.allow_all_tools")}</span>
                  <Switch
                    checked={form.toolsAllowAll}
                    onCheckedChange={(v) => upd({ toolsAllowAll: v })}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Field label={t("agents.field_allowed")}>
                    <ToolMultiSelect
                      tools={toolNames}
                      selected={form.toolsAllow.split(",").map((s) => s.trim()).filter(Boolean)}
                      onChange={(v) => upd({ toolsAllow: v.join(", ") })}
                      placeholder={t("common.select_tools_placeholder")}
                    />
                  </Field>
                  <Field label={t("agents.field_denied")}>
                    <ToolMultiSelect
                      tools={toolNames}
                      selected={form.toolsDeny.split(",").map((s) => s.trim()).filter(Boolean)}
                      onChange={(v) => upd({ toolsDeny: v.join(", ") })}
                      placeholder={t("common.select_tools_placeholder")}
                    />
                  </Field>
                </div>
            ...
            {/* Heartbeat */}
            <SwitchSection
              title={t("agents.section_schedule")}
              enabled={form.hbEnabled}
              onToggle={(v) => upd({ hbEnabled: v })}
            >
              <CronSchedulePicker
                value={form.hbCron}
                onChange={(v) => upd({ hbCron: v })}
                timezone={form.hbTimezone || "UTC"}
                onTimezoneChange={(v) => upd({ hbTimezone: v })}
              />
            ...
            <SwitchSection
              ...
              enabled={form.tlEnabled}
              onToggle={(v) => upd({ tlEnabled: v })}
            >
              <div className="space-y-2">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <Field label={t("agents.field_tl_warn_threshold")}>
                    <Input
                      ...
                      min="1"
                      className="bg-background border-border font-mono text-sm h-8"
                      value={form.tlWarnThreshold}
                      onChange={(e) => upd({ tlWarnThreshold: e.target.value })}
                    />
                  </Field>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    checked={form.tlCompactOnOverflow}
                    onCheckedChange={(v) => upd({ tlCompactOnOverflow: v })}
                    className="data-[state=checked]:bg-primary"
                  />
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    checked={form.tlDetectLoops}
                    onCheckedChange={(v) => upd({ tlDetectLoops: v })}
                    className="data-[state=checked]:bg-primary"
                  />
                </div>
              </div>
            ...
            {/* Approval */}
            <SwitchSection
              title={t("agents.section_approval")}
              enabled={form.approvalEnabled}
              onToggle={(v) => upd({ approvalEnabled: v })}
            >
              <div className="space-y-2">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    <div className="flex flex-col gap-1.5">
                      {(["system", "destructive", "external"] as const).map((cat) => (
                        <label key={cat} className="flex items-center gap-2 text-xs cursor-pointer">
                          <input
                            type="checkbox"
                            checked={form.approvalCategories.includes(cat)}
                            onChange={(e) => {
                              const next = e.target.checked
                                ? [...form.approvalCategories, cat]
                                : form.approvalCategories.filter((c: string) => c !== cat);
                          ...
                          <span className="text-muted-foreground/60 text-[10px]">
                            {cat === "system" ? "(shell, code, git)" :
                             cat === "destructive" ? "(write, delete, edit)" :
                             "(all other tools)"}
                          </span>
                        </label>
                      ))}
                  ...
                  <Field label={t("agents.approval_timeout")}>
                    <Input
                      ...
                      max="3600"
                      step="30"
                      value={form.approvalTimeout}
                      className="bg-background border-border font-mono text-sm h-8"
                      onChange={(e) => upd({ approvalTimeout: e.target.value })}
                    />
                ...
                <Field label={t("agents.approval_specific_tools")}>
                  <ToolMultiSelect
                    tools={toolNames}
                    selected={form.approvalRequireFor}
                    onChange={(v) => upd({ approvalRequireFor: v })}
                    placeholder={t("agents.approval_tools_placeholder")}
                  />
                </Field>
              </div>
            </SwitchSection>
            ...
            {/* Session */}
            <SwitchSection
              title={t("agents.section_session")}
              enabled={form.sessionEnabled}
              onToggle={(v) => upd({ sessionEnabled: v })}
            >
              <div className="space-y-2">
                <Field label={t("agents.field_dm_scope")}>
                  <Select
                    value={form.sessionDmScope}
                    onValueChange={(v) => upd({ sessionDmScope: v })}
                  >
                    <SelectTrigger className="w-full bg-background border-border text-sm h-8">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Field label={t("agents.field_ttl_days")}>
                    <Input
                      ...
                      onChange={(e) => upd({ sessionTtlDays: e.target.value })}
                    />
                  </Field>
                  <Field label={t("agents.field_max_messages")}>
                    <Input
                      ...
                      min="0"
                      className="bg-background border-border font-mono text-sm h-8"
                      value={form.sessionMaxMessages}
                      onChange={(e) => upd({ sessionMaxMessages: e.target.value })}
                    />
                  </Field>
                  <Field label={t("agents.field_prune_tool_output")}>
                    <Input
                      type="number" step="1" min="0"
                      value={form.sessionPruneToolOutput} placeholder="Off"
                      className="bg-background border-border font-mono text-sm h-8"
                      onChange={(e) => upd({ sessionPruneToolOutput: e.target.value })}
                    />
                  </Field>
                  <Field label={t("agents.field_max_history")}>
                    <Input
                      type="number" step="1" min="0"
                      value={form.maxHistoryMessages} placeholder="Unlimited"
                      className="bg-background border-border font-mono text-sm h-8"
                      onChange={(e) => upd({ maxHistoryMessages: e.target.value })}
                    />
                  </Field>
                </div>
              </div>
            ...
            {/* Hooks */}
            <SwitchSection
              title={t("agents.section_hooks")}
              enabled={form.hooksLogAll || form.hooksBlockTools.trim() !== ""}
              onToggle={(v) => { if (!v) upd({ hooksLogAll: false, hooksBlockTools: "" }); else upd({ hooksLogAll: true }); }}
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                ...
                <Field label={t("agents.hooks_block_tools")}>
                  <Input
                    value={form.hooksBlockTools} placeholder="tool1, tool2"
                    className="bg-background border-border font-mono text-sm h-8"
                    onChange={(e) => upd({ hooksBlockTools: e.target.value })}
                  />
                </Field>
              </div>
            </SwitchSection>
            ...
            {/* Access Control */}
            <SwitchSection
              title={t("agents.section_access")}
              enabled={form.accessEnabled}
              onToggle={(v) => upd({ accessEnabled: v })}
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {form.accessMode === "restricted" && (
                  <Field label={t("agents.field_access_owner_id")}>
                    <Input
                      value={form.accessOwnerId}
                      placeholder="Telegram User ID"
                      className="bg-background border-border font-mono text-sm h-8"
                      onChange={(e) => upd({ accessOwnerId: e.target.value })}
                    />
                  </Field>
                )}
              </div>
            ...
            <div className="space-y-2 border-t border-border/30 pt-3">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t("agents.section_budget")}</h3>
              <Field label={t("agents.field_daily_budget")}>
                <Input
                  type="number" step="10000" min="0"
                  value={form.dailyBudgetTokens}
                  className="bg-background border-border font-mono text-sm h-8"
                  onChange={(e) => upd({ dailyBudgetTokens: e.target.value })}
                />
              </Field>
            </div>
            ...
            <RoutingRulesEditor
              ...
              onChange={(routing) => upd({ routing })}
            />

            {/* Channels -- read-only summary, manage on Channels page */}
            {editName && (
              <div className="space-y-2 border-t border-border/30 pt-3">
                <div className="flex items-center justify-between">
